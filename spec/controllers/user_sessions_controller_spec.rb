  ...
  describe "With Ajax" do
    ...
    describe "logging in with valid credentials" do
      ...
      it "should refresh the dashboard" do
        do_login
        response.body.should include(%Q[Toolbox.frames(":resource(user_sessions/show)").refresh()])
      end

      it "should remove the login frame" do
        do_login
        response.body.should include(%Q[Toolbox.frames(":resource(user_sessions/new)").remove()])
      end
